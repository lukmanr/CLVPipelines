...
def clv_train_pipeline_dataproc_automl(project_id,
  ...
  import_dataset_op = kfp.components.load_component_from_file(
      AML_IMPORT_DATASET_SPEC_URI)
  train_model_op = kfp.components.load_component_from_file(
      AML_TRAIN_MODEL_SPEC_URI)
  retrieve_metrics_op = kfp.components.load_component_from_file(
      AML_RETRIEVE_METRICS_SPEC_URI)
  ...
  delete_cluster_exit_handler = dataproc_delete_cluster_op(
      project_id=project_id, region=compute_region, name=cluster_name)
  ...
  with dsl.ExitHandler(exit_op=delete_cluster_exit_handler):
    ...
    with dsl.Condition(
        retrieve_metrics_task.outputs['output_mae'] < mae_threshold):
